...
## Phase 3A: MVP 운영 준비 확장

### 3A.1 기반 재정비
- [x] T201 Next.js · Tailwind · Prisma 설정 점검 및 `README.md` 설치 절차 보완
- [x] T202 `.env.example` 보강 및 비밀키 취급 가이드 (`docs/configuration.md`)
...
- [ ] T221 외부 API 재시도·백오프 유틸 추가 (`src/lib/external-client.ts`, `tests/unit/external-client.test.ts`)
- [ ] T222 Taobao → 쿠팡 통합 테스트 갱신 (`tests/integration/product-registration.test.ts`)

### 3A.4 백그라운드 동기화 & 관측성
- [ ] T230 재고/가격 동기화 잡 스케줄링 (`src/lib/cron/inventory-sync.ts`, `tests/unit/cron/inventory-sync.test.ts`)
- [ ] T231 번역 캐시 정책 및 비용 가드 구현 (`src/services/translation.service.ts`, `tests/unit/translation.service.test.ts`)
- [ ] T232 BullMQ 큐 로깅/메트릭 삽입 (`src/lib/queue/translation-queue.ts`, `docs/runbooks/queues.md`)

### 3A.5 UI 사용자 여정
- [ ] T240 GraphQL Mutation을 `src/app/products/page.tsx`에 연결하고 성공/에러 토스트 제공
- [ ] T241 `src/components/product/ProductForm.tsx`에 단계별 진행 상태/검증 메시지 추가
- [ ] T242 Playwright E2E 시나리오로 Taobao → 쿠팡 흐름 검증 (`tests/e2e/product-ingestion.spec.ts`)

### 3A.6 품질 게이트 & 문서화
- [ ] T250 운영 런북 작성 (`docs/runbooks/taobao-to-elevenst.md`)
- [ ] T251 커버리지 70% 이상 CI 체크 설정 (`package.json`, `.github/workflows/test.yml`)
- [ ] T252 API 응답/큐 딜레이 스모크 테스트 (`tests/performance/api-smoke.test.ts`)
- [ ] T253 README · AGENTS · quickstart 갱신으로 최신 MVP 흐름 반영
